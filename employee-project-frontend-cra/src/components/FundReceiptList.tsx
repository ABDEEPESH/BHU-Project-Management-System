...
const FundReceiptList: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.log('[FundReceiptList] Receipts fetched successfully', { count: fetchedReceipts.length });
      } catch (err) {
        const errorMessage = err instanceof Error ? err.message : 'Failed to fetch fund receipts';
        setError(errorMessage);
        console.error('[FundReceiptList] Error fetching receipts:', err);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleDelete = async (receiptNumber: string) => {
    if (window.confirm(`Are you sure you want to delete the fund receipt with number "${receiptNumber}"?`)) {
      try {
        await deleteFundReceipt(receiptNumber);
        setReceipts((prev) => prev.filter((receipt) => receipt.receiptNumber !== receiptNumber));
        console.log('[FundReceiptList] Fund receipt deleted successfully', { receiptNumber });
      } catch (error) {
        console.error('[FundReceiptList] Error deleting fund receipt', { error });
        setError('Failed to delete fund receipt.');
      }
    }
  };
        ...
        <div className="col-md-8">
          <div className="input-group">
            ...
            <input
              ...
              placeholder="Search by any field (Receipt Number, EID, Project Number, etc.)"
              value={searchQuery}
            ...
            <tbody>
              {filteredReceipts.length === 0 ? (
                ...
                filteredReceipts.map((receipt, index) => (
                  <tr key={receipt.receiptNumber || `receipt-${index}`}>
